...
public class Heart : MonoBehaviour
{
    ...
    /*
            ...
            endPosition[i] = new Vector3(
                Mathf.Sqrt(2f) * sin * sin * sin,
                -1 * cos * cos * cos - cos* cos + 2 * cos, 10f
            );
*/
            ...
            for (int i = 0; i < numSphere; i++)
            {
                ...
                // Heart shape formula
                float x = Mathf.Sqrt(2f) * sin * sin * sin;
                float y = -cos * cos * cos - cos * cos + 2 * cos;
    ...
    void Update()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            */
            ...
            // white fading into pink and then to red
            float hue = 0.95f + 0.05f * ((float)i / numSphere); 
            float saturation = 0.5f + 0.5f * Mathf.Sin(time);
            float brightness = 1f;

            Color color = Color.HSVToRGB(hue % 1f, Mathf.Abs(saturation), brightness);
